...
LSHA256DEF int lonesha256_fixed(unsigned char[32], const unsigned char*, size_t);
LSHA256DEF int lonesha256_stream(unsigned char[32], int(*)(unsigned char*), size_t);
...
//lonesha256_fixed function
LSHA256DEF int lonesha256_fixed (unsigned char out[32], const unsigned char* in, size_t len) {
    //writes the sha256 hash of the first "len" bytes in buffer "in" to buffer "out"
    //returns 0 on success, may return non-zero in future versions to indicate error
    const uint32_t K[64] = {
